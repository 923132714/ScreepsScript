...
export default class FactoryExtension extends StructureFactory {
  ...
  private prepare(): void {
    ...
    for (const resType in subResources) {
      ...
      if (subResources.hasOwnProperty(resType)) {
        ...
        const subResAmount = FactoryExtension.clacSubResourceAmount(
          task.target,
          task.amount,
    ...
    for (const resType in subResources) {
      ...
      if (subResources.hasOwnProperty(resType)) {
        ...
        if (this.store[resType] < subResources[resType]) {
          ...
          // 合成任务需要的该材料数量
          const needAmount = FactoryExtension.clacSubResourceAmount(
            task.target,
            task.amount,
            resType as ResourceConstant
          );
  ...
  private canContinueProduce(): boolean {
    ...
    for (const res in subRes) {
      // eslint-disable-next-line no-prototype-builtins
      if (subRes.hasOwnProperty(res) && this.store[res] < subRes[res]) return false;
    }
  ...
  private putResource(): void {
    ...
    for (const resType in this.store) {
      // eslint-disable-next-line no-prototype-builtins
      if (this.store.hasOwnProperty(resType)) {
        // 是目标产物的话就更新统计信息
        ...
        this.room.centerTransport.addTask({
          ...
          resourceType: resType as ResourceConstant,
          amount: this.store[resType as ResourceConstant]
        });
        return;
      }
    }
   ...
   */
  private static clacSubResourceAmount(
    targetResource: CommodityConstant,
    targetAmount: number,
    subResource: ResourceConstant
  ): number {
  ...
  private addTask(task: IFactoryTask = undefined): number {
    ...
    if (
      this.room.terminal &&
      topTarget in commodityMax &&
      this.room.terminal.store[topTarget] >= commodityMax[topTarget]
    ) {
      let targetIndex = 0;
      // 修正预定目标
      ...
      // 找到了，按照其索引更新下次预定索引
      else this.room.memory.factory.targetIndex = targetIndex + 1 >= topTargets.length ? 0 : targetIndex + 1;
    }
    // 没有到达上限，按原计划更新索引
    else
      this.room.memory.factory.targetIndex = memory.targetIndex + 1 >= topTargets.length ? 0 : memory.targetIndex + 1;
